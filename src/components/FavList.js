...
function FavList() {
  ...
  return (
    <div className="lg:ml-64 md:ml-64 w-full px-4 lg:px-10 py-5">
    ...
    {favorites.length > 0 ? (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4">
        {favorites.map((item) => (
          <div
            key={item.id}
            className=" py-2 lg:p-4 border rounded-lg flex justify-center items-center border-secondary bg-white shadow-md"
          >
            <Image
              src={item.imageUrl}
              alt={item.name}
